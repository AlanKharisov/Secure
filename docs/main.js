import { API, Auth, api, qs, qsa, h, flash, makeQR, downloadCanvasPng } from "./app.js";
...
function productCard(p, opts={}) {
    const url = p.publicUrl || `${API}/details.html?id=${p.tokenId}`;
    ...
    const copyBtn = h("button", {class:"copy", onclick(){
            navigator.clipboard.writeText(String(p.tokenId));
            flash("tokenId скопійовано");
        }}, "Скопіювати tokenId");

    const dlBtn = h("button", {class:"copy", onclick(){
...
function setupUserCreate() {
    qs("#userCreateForm").addEventListener("submit", async (e) => {
        e.preventDefault();
        ...
        const payload = {
            ...
            certificates: (fd.get("certs")||"").split(",").map(s=>s.trim()).filter(Boolean),
        };
...
function setupCompanyCreate() {
    qs("#companyCreateForm").addEventListener("submit", async (e) => {
        e.preventDefault();
        const fd = new FormData(e.currentTarget);
        const payload = {
            name: fd.get("name"),
