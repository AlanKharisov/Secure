(function () {
  ...
  function add(item){ const items = read(); if (!items.some(x => Number(x.id) === Number(item.id))) { items.push(item); save(items); } }
  function remove(id){ const items = read().filter(x => Number(x.id) !== Number(id)); save(items); }
  function clear(){ save([]); }
  function updateBadge(){ const el = document.getElementById("cartCount"); if (el) el.textContent = String(count()); }

  function addPurchases(items){
    try {
      const cur = JSON.parse(localStorage.getItem(PURCHASES_KEY) || "[]");
      items.forEach(it => { if (!cur.some(x => Number(x.id) === Number(it.id))) cur.push(it); });
      localStorage.setItem(PURCHASES_KEY, JSON.stringify(cur));
      // підкажемо акаунт-сторінці оновитись
      if (window.__MARKI__ && typeof window.__MARKI__.addLocalPurchase === "function"){
        items.forEach(window.__MARKI__.addLocalPurchase);
      }
    } catch {}
  }
  ...
  async function checkout(){
      ...
      try{
        ...
        if(!r.ok){ fail.push({it, err: j.error || "error"}); }
          ...
          ok.push({
            ...
            serial: it.serial,
            image: it.image,
            publicUrl: it.url || `${API}/details.html?id=${it.id}`,
            state: "purchased",
    ...
    if (ok.length > 0) addPurchases(ok);
